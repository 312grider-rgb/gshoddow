...
(function () {
  ...
  function withTimeout(promise, ms, label) {
    return Promise.race([
      promise,
      new Promise((_, reject) => setTimeout(() => reject(new Error((label || 'Request') + ' timed out — check your connection and try again.')), ms))
    ]);
  }
  function genCode() {
  ...
  const SSClass = {
    async create({ name, subject, description }) {
      ...
      do {
        code = genCode();
        const { data } = await withTimeout(client().from('classrooms').select('id').eq('code', code).maybeSingle(), 12000, 'Checking code');
        exists = !!data; tries++;
      } while (exists && tries < 8);
      const { data, error } = await withTimeout(client().from('classrooms').insert({
        code, name, subject: subject || '', description: description || '', teacher_id: user.id
      }).select().single(), 12000, 'Creating classroom');
      if (error) return { ok: false, error: error.message };
      const classroom = await withTimeout(hydrateClassroom({ ...data, teacher_name: user.name }), 12000, 'Loading classroom');
    ...
    async myClasses() {
      const user = await currentUserOrFail();
      const [{ data: teaching }, { data: memberRows }] = await withTimeout(Promise.all([
